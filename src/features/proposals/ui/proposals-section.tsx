    ...
    >
      <div className="grid w-full grid-cols-1 gap-8 md:gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-12">
        <div>
          ...
          <h2 className="mt-5 font-heading text-[clamp(2rem,7vw,3.5rem)] font-extrabold leading-[1.05] sm:mt-6">
            {proposals.title}
          </h2>
          <p className="mt-4 max-w-[48ch] text-[16px] leading-[1.45] sm:text-[18px]">{proposals.subtitle}</p>
        ...
        <AnimatePresence mode="wait">
          <motion.article
            ...
            className="grid overflow-hidden rounded-[24px] border border-[#ffffff3d] bg-[#ffffff12] shadow-[0_20px_55px_rgba(0,0,0,0.35)] sm:rounded-[30px]"
          >
            ...
            <div className="px-5 py-6 sm:px-7 sm:py-7 lg:px-10 lg:py-9">
              <span className="inline-flex rounded-full bg-brandPinkLight px-3 py-2 text-[12px] font-semibold text-white sm:px-4 sm:text-[13px]">
                {current.tabLabel} - {current.category}
              </span>
              <h3 className="mt-4 font-heading text-[clamp(1.7rem,6vw,3rem)] font-extrabold leading-[1.05] text-whiteMain sm:mt-5">
                {current.title}
              </h3>
              <p className="mt-4 text-[16px] leading-[1.45] text-textMuted sm:text-[18px]">{current.summary}</p>

              <ul className="mt-6 grid gap-2">
                {current.impacts.map((impact) => (
                  <li key={impact} className="flex items-center gap-3 text-[15px] text-whiteMain sm:text-[16px]">
